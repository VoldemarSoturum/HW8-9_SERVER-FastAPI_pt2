...
В нашем проекте дополнительно реализован **bootstrap‑пользователь root через env**, чтобы было удобно “поднять” первого администратора.  

- По смыслу прав доступа **root эквивалентен admin** (может всё).
- **Для выполнения задания наличие `root` не требуется.**
- Если нам нужна строгая реализация “только user/admin”, просто **не задаём** переменные `BOOTSTRAP_ROOT_USERNAME/BOOTSTRAP_ROOT_PASSWORD` и **не используйте** группу `root` в запросах/тестах.
